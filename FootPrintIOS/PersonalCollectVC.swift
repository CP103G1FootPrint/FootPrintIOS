...
class PersonalCollectVC: UICollectionViewController {
    
//    let userDefault = UserDefaults.standard
    let url_server = URL(string: common_url + "CollectServlet")
    var collectionImageId = [Collect]()
    let user = loadData()
    ...
    override func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        return collectionImageId.count
    }
    
    override func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "CollectCell", for: indexPath) as! PersonalCollectionTVCell
        let imageId = collectionImageId[indexPath.row]
        ...
        executeTask(url_server!,requestParam) { (data, response, error) in
            if error == nil {
                ...
                DispatchQueue.main.async { cell.CollectionImage.image = image }
            } else {
                print(error!.localizedDescription)
            }
        }
